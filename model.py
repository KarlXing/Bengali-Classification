...
class SENetHeavyHead(nn.Module):

    def __init__(self, block, layers, groups, reduction, dropout_p=0.2,
                 inplanes=128, input_3x3=True, downsample_kernel_size=3,
                 downsample_padding=1, num_classes=186):
        super(SENetHeavyHead, self).__init__()
        self.inplanes = inplanes
        if input_3x3:
            layer0_modules = [
                ('conv1', nn.Conv2d(1, 64, 3, stride=2, padding=1,
        ...
        self.avg_pool = nn.AvgPool2d(1, stride=1)
        self.dropout = nn.Dropout(dropout_p) if dropout_p is not None else None
        self.last_linear1 = nn.Linear(32768, 10000)
        self.last_linear2 = nn.Linear(10000, 1000)
        self.last_linear3 = nn.Linear(1000, num_classes)
